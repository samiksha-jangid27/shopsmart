...
export default async function HomePage() {
  const data = await apiFetch<HomeData>("/home");
    ...
    <>
      ...
      <section className="container-padded py-16">
        <div className="mb-10 flex items-end justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-smoke">New arrivals</p><h2 className="mt-3 font-serif text-4xl lg:text-5xl">Fresh from the rail</h2></div>
        </div>
        <ProductGrid products={data.newArrivals} />
      </section>
      <EditorialBlocks />
      <section className="container-padded py-16">
        <div className="mb-10"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-smoke">Best sellers</p><h2 className="mt-3 font-serif text-4xl lg:text-5xl">Most requested pieces</h2></div>
      ...
      <section className="container-padded grid gap-5 py-16 md:grid-cols-3">
        {["Maison Luma", "Atelier Aven", "Loom Studio"].map((brand) => (
          <article key={brand} className="border border-oat bg-white p-8">
            ...
            <p className="mt-4 text-sm leading-7 text-smoke">Selected for material integrity, modern proportion, and pieces that last past one season.</p>
          </article>
        ))}
      </section>
